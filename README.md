...
### Gameplay HUD — status bars, vehicle dash, and every shared overlay in the framework
...
The HUD system includes: status bars, vehicle dash, buffs, and compass, plus every on-demand overlay component that the framework drives through - such as targeting reticle, radial interaction menu, list menus, confirm/input dialogs, progress bars, notifications, settings, and screen effects

> [!WARNING]
> This is the most widely used resource in the framework - other resources call into it via `plsr.Notification`, `plsr.Confirm`, `plsr.Input`, `plsr.List`, `plsr.InfoOverlay`, `plsr.Interaction`, and more. Changing a message name or payload shape here can break every resource that calls into it, not just this one

---

## Theming

HUD layout is a per-character server-persisted setting, not local storage — edits go through the Settings dialog and save server-side

Edit `ui/src/theme.css` for colors and fonts and `ui/src/config.ts` for the default HUD config (keep it in sync with the server's seed default in `server/server.lua`), then rebuild:
